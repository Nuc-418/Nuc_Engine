...
| **Maps** (right, tabbed with Outliner) | All maps in `assets/scenes`. **+ New Map** creates an empty map (ambient environment term + a default **Sun** directional Light actor) and switches to it; **double-click** switches maps; right-click offers Load and **Delete** (with confirmation). Switching discards unsaved changes — Ctrl+S first. Deleting the open map keeps the world in memory; saving recreates the file. |
| **Outliner** (right) | The scene hierarchy as a tree. Click to select, right-click or **Del** to delete, **+ Add** spawns a Cube / IndexedCube / IronMan in front of the camera. **Drag an object onto another** to attach it (world pose kept, undoable); drag onto the empty area below — or right-click > Detach from parent — to detach to the root. Deleting a parent reparents its children. **Right-click > Save as Prefab** stores the object (components + rotation/scale) in `assets/prefabs` and makes it a spawnable type immediately. |
| **Details** (right, below) | Name and Position / Rotation (degrees) / Scale of the selection, live. |
| **Environment** (bottom) | The world ambient term (color + on). Directional/point/spot lights are **Light components** — add a Light actor (Outliner ▸ + Add ▸ Light) and edit it in Details; both the model and primitive shaders support 50 point + 50 spot lights each. |
